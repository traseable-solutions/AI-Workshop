                ...
                <form method="POST" action="{{ route('leave-requests.store') }}" enctype="multipart/form-data" class="space-y-4" x-data="{ type: '{{ old('type', 'annual') }}' }">
                    ...
                    <div x-show="type === 'annual'" class="space-y-4 border-t pt-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="leave_destination" name="leave_destination" type="text" class="mt-1 block w-full" :value="old('leave_destination')" x-bind:required="type === 'annual'" />
                            <x-input-error :messages="$errors->get('leave_destination')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="leave_address" value="Leave Address" />
                            <x-text-input id="leave_address" name="leave_address" type="text" class="mt-1 block w-full" :value="old('leave_address')" x-bind:required="type === 'annual'" />
                            <x-input-error :messages="$errors->get('leave_address')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="phone_contact" value="Phone Contact" />
                            <x-text-input id="phone_contact" name="phone_contact" type="text" class="mt-1 block w-full" :value="old('phone_contact')" x-bind:required="type === 'annual'" />
                    ...
                    <div x-show="type === 'annual'" class="space-y-4 border-t pt-4">
                        ...
                        <div>
                            ...
                            <select id="level" name="level" x-bind:disabled="type !== 'annual'" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                @for ($i = 1; $i <= 12; $i++)
                                    <option value="{{ $i }}" @selected(old('level') == $i)>Level {{ $i }}</option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select id="package_amount" name="package_amount" x-bind:disabled="type !== 'annual'" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                <option value="8000" @selected(old('package_amount') == 8000)>$8,000</option>
                                <option value="10000" @selected(old('package_amount') == 10000)>$10,000</option>
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="travel_expense_assistance" name="travel_expense_assistance" type="number" min="0" class="mt-1 block w-full" :value="old('travel_expense_assistance')" />
